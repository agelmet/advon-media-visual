...
// Only posts Angelo APPROVED in the CRM Meta tab are published; drafts and skipped posts never are.
...
import { readState, patchPost, kick } from '../../lib/socialrun.js';
...
export default async (req) => {
  ...
  const report = { at: new Date().toISOString(), due: due.map((p) => p.id), stuck: stuck.map((p) => p.id), dry };
  if (!dry) {
