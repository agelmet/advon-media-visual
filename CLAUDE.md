...
- Analytics: GA4 property `ADVON MEDIA` (account 407168770, property 553173436), Measurement ID `G-2JJP554SVE`, mounted as `components/Analytics.jsx` in `app/layout.jsx` on **both** `main` and `v2-draft`. Loads after paint (never touches LCP), sends its own `page_view` on each app-router navigation, skips `/crm`, `/crm-demo`, `/leads`, `/api`, and excludes a browser for good after one visit to `/?me=1`. Measurement IDs are public — safe in the client bundle.
