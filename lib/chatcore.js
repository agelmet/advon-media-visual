...
// ---------- the client layout ----------
const NEXT_STEP = {
  yliko: 'Στείλτε μας το υλικό σας μαζεμένο — σε ένα μήνυμα ή σε ένα αρχείο Word. Μόλις τα έχουμε όλα, ξεκινάμε.',
  draft: 'Δείτε το draft με την ησυχία σας και γράψτε μας όλες τις αλλαγές μαζί, αριθμημένες (1., 2., 3.).',
  changes: 'Ρίξτε μια ματιά στις αλλαγές. Αν είναι όλα όπως τα θέλετε, πείτε μας «προχωράμε» και τη δημοσιεύουμε.',
  live: '',
};
...
const REMIND = {
  yliko: ['Μια μικρή υπενθύμιση για το υλικό σας', 'Για να ξεκινήσουμε την ιστοσελίδα σας χρειαζόμαστε λίγο υλικό — φωτογραφίες, το λογότυπο (αν υπάρχει) και δυο λόγια για εσάς. Στείλτε τα από τη σελίδα συνομιλίας μας όποτε σας βολεύει, κι εμείς ξεκινάμε αμέσως.'],
  draft: ['Είδατε το πρώτο draft;', 'Το πρώτο draft της ιστοσελίδας σας σας περιμένει. Δείτε το με την ησυχία σας και γράψτε μου στη συνομιλία ό,τι θέλετε να αλλάξουμε — όσες αλλαγές χρειαστούν μέχρι να είναι ακριβώς όπως τη θέλετε. Μόλις το εγκρίνετε, προχωράμε στη δημοσίευση.'],
  changes: ['Είναι όλα όπως τα θέλετε;', 'Οι τελευταίες αλλαγές είναι έτοιμες στη σελίδα σας. Αν είναι όλα όπως τα θέλετε, γράψτε μου «προχωράμε» στη συνομιλία και τη δημοσιεύουμε — αλλιώς πείτε μου τι άλλο να αλλάξω.'],
};
...
export function clientWroteNote(t, fresh) {
  ...
  const next = 'Nothing is needed right now. Claude reads it at the next check (09:05 and 17:05), does the work on the draft and sends you the reply to approve. You can also answer yourself at any time.';
  const blocks = [
    { h: 'What they wrote', quote: cut(said, 1800), tone: 'blue' },
    files.length ? { h: 'Files', lines: files.map((f) => f.name), tone: 'grey' } : null,
    { h: 'What happens next', text: next, tone: 'grey' },
  ];
  return {
    tg: `💬 <b>${tgEsc(upper(t.name))}</b> wrote\nStage: <b>${tgEsc(stageEn(t.stage))}</b> · ${tgEsc(sub)}${tgBlock('What they wrote', tgQuote(said))}${files.length ? tgBlock('Files', tgList(files.slice(0, 8).map((f) => f.name))) : ''}${tgBlock('What happens next', tgEsc('Claude prepares the reply at the next check (09:05 / 17:05) and asks for your OK — or answer now yourself.'))}`,
    buttons: [{ text: 'Open the chat', url: crmLink(t) }],
    subject: `💬 ${t.name} wrote · ${stageEn(t.stage)}`,
    text: `${t.name} wrote — stage ${stageEn(t.stage)} · ${sub}${textBlocks(blocks)}\n\nOpen the chat: ${crmLink(t)}`,
    html: angeloMail({ badge: 'New message', badgeColor: '#2F7FE0', name: t.name, stage: stageEn(t.stage), sub, blocks, t }),
  };
}
