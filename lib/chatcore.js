...
function richText(text, link) {
  let h = esc(text);
  // [Όχι — ξεκινήστε](#start) · [Ναι, θα στείλω κι άλλα](#more) → two real buttons that open the chat page (#start presses it there)
  h = h.replace(/\[([^\]\n]{1,80})\]\(#(start|more)\)/g, (_, label, act) => `<a href="${link}${act === 'start' ? '#start' : ''}" style="display:inline-block;margin:4px 6px 4px 0;padding:11px 16px;border-radius:10px;font-weight:700;text-decoration:none;${act === 'start' ? 'background:#2F7FE0;color:#ffffff' : 'background:#E8F0FB;color:#2F7FE0'}">${label}</a>`);
  h = h.replace(/(<\/a>)\r?\n(?=<a href="[^"]*" style="display:inline-block)/g, '$1');
...
const plainText = (text) => String(text || '').replace(/\[([^\]\n]{1,80})\]\(#(odigos|extras|yliko|start|more)\)/g, '$1');
// Angelo's saved wording for the standard messages (chats/templates.json → {t:{key:text}, extra:[[title,text]]})
export async function readTemplatesAll(conf) { try { const d = await readChatJson(conf, 'templates.json', {}); return { t: (d && typeof d.t === 'object' && d.t) || {}, extra: (d && Array.isArray(d.extra) && d.extra) || [] }; } catch { return { t: {}, extra: [] }; } }
export async function readTemplates(conf) { return (await readTemplatesAll(conf)).t; }
