...
export function newMessageMail(t, text, files) {
  ...
  const body = `<p style="margin:0 0 12px">${greeting()}! Σας έγραψα στη σελίδα συνομιλίας μας για την ιστοσελίδα σας:</p>
  ...
  return {
    ...
    text: `${greeting()}! Σας έγραψα στη σελίδα συνομιλίας μας για την ιστοσελίδα σας:\n\n${preview || '(συνημμένο αρχείο)'}\n\nΑνοίξτε τη συνομιλία: ${link}\n\nΆγγελος · Advon Media`,
    html: wrap('Νέο μήνυμα για την ιστοσελίδα σας', body, link, 'Ανοίξτε τη συνομιλία'),
  };
}
const REMIND = {
  yliko: ['Μια μικρή υπενθύμιση για το υλικό σας', 'Μια φιλική υπενθύμιση: για να ξεκινήσουμε την ιστοσελίδα σας χρειαζόμαστε λίγο υλικό — φωτογραφίες, το λογότυπο (αν υπάρχει) και δυο λόγια για εσάς. Στείλτε τα από τη σελίδα συνομιλίας μας όποτε σας βολεύει, κι εμείς ξεκινάμε αμέσως.'],
...
export function reminderMail(t) {
  const r0 = REMIND[t.stage]; if (!r0) return null;
  const r = [r0[0], `${greeting()}! ${r0[1]}`];
  const link = chatLink(t);
