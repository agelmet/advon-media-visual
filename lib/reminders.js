...
// «📌 …» events = things Angelo has to do himself → the CRM puts them in «Do ASAP» on (or after) their day, once.
// «📌🔥 …» (or 🔥 anywhere in the title) = super-high priority. Added by Claude from any chat, or by Angelo in the Calendar app.
const ASAP_RE = /^\s*📌\s*(.*)$/u;
export function readAsap(ev) {
  const summary = unescapeIcs(ev.SUMMARY && ev.SUMMARY.value);
  const m = summary.match(ASAP_RE); if (!m) return null;
  const start = parseDt(ev.DTSTART); if (!start) return null;
  const hot = /🔥/u.test(m[1]);
  const text = m[1].replace(/🔥/gu, '').replace(/^\s*[—–:-]\s*/, '').trim();
  if (!text) return null;
  return { uid: String((ev.UID && ev.UID.value) || '').trim() || `${start.day}|${summary}`, day: start.day, text: text.slice(0, 240), hot, status: String((ev.STATUS && ev.STATUS.value) || 'CONFIRMED').toUpperCase() };
}
export function parseAsap(icsText) {
  const blocks = unfold(icsText).split('BEGIN:VEVENT').slice(1).map((b) => b.split('END:VEVENT')[0]);
  const seen = new Set(); const out = [];
  for (const b of blocks) {
    let r = null; try { r = readAsap(parseBlock(b)); } catch { r = null; }
...
export async function fetchReminders() { return parseReminders(await fetchCalendarText()); }
