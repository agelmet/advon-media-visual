...
// Auto-post (Angelo, 26 Sept 2026): every post goes out on its own date unless he skips it or takes the approval back.
// settings.auto defaults to ON; a post's own approved:false (set by «Take back approval») always wins.
export const autoOn = (state) => !(state && state.settings && state.settings.auto === false);
export function merged(plan, state) {
  const S = (state && state.posts) || {};
  const auto = autoOn(state);
  return (plan.posts || []).map((p) => {
    const s = S[p.id] || {};
    const date = s.date || p.date, time = s.time || p.time || '19:30';
    const approved = s.skip ? false : (s.approved === true || (auto && s.approved !== false));
    return { ...p, date, time, caption: s.caption != null ? s.caption : p.caption, hashtags: s.hashtags != null ? s.hashtags : p.hashtags,
      approved, skip: !!s.skip, status: s.status || (s.skip ? 'skipped' : approved ? 'approved' : 'draft'), month: String(p.date).slice(0, 7),
