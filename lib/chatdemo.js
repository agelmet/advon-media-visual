...
export const YLIKO_DAT = 'Στάδιο 1 · Υλικό 📷\n\nΤα κείμενα και τα στοιχεία σας τα παίρνουμε από το προφίλ σας στο doctoranytime — δεν χρειάζεται να γράψετε τίποτα.\n\nΧρειαζόμαστε μόνο τις φωτογραφίες σας σε καλή ανάλυση: εσείς, ο χώρος σας και το λογότυπο αν υπάρχει. Στείλτε τις εδώ, στη συνομιλία.\n\nΜόλις τις στείλετε, πατήστε «Ξεκινήστε την ιστοσελίδα μου» — το πρώτο draft έρχεται σε 5–10 ημέρες. ⏱️';
...
export function demoSeed(stage, t, dat) {
  ...
  if (s === 'draft') return [WELCOME, fill(FIRST_DRAFT, t)];
  if (s === 'changes') return [WELCOME, fill(CHANGES_DONE, t, { changes: '1. Η φωτογραφία στην αρχή άλλαξε με τη νέα\n2. Το τηλέφωνο διορθώθηκε στην επικοινωνία και στο υποσέλιδο\n3. Το κείμενο «Λίγα λόγια για εμένα» ενημερώθηκε' })];
  if (s === 'live') return [WELCOME, fill(DOMAIN_LIVE, t, { domain: 'https://' + DEMO_DOMAIN })];
  return [WELCOME, dat ? YLIKO_DAT : YLIKO_PLAIN];
}
...
export function demoReply(t, msg) {
  ...
  if (kind === 'live') {
    ...
    return [{ text: fill(DOMAIN_FREE, t, { domain }), clearReady: true }];
  }
  ...
  if ((stage === 'draft' || stage === 'changes') && /(^|\n)\s*\d+[.)]\s*\S/.test(text)) return [{ text: NUDGE_CHANGES }];
